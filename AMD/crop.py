...
# --- Configuration ---
STATE_DICT_PATH = "model/produce/mobilenetv3small_produce.pth" # Correct weights file
CLASS_NAMES_FILE = "model/class_names_produce.txt"
IMAGE_PATH = "assets/cucumber3.jpeg" # Or your test image like "carrot.jpg"
# Standard ImageNet preprocessing values (MobileNetV3 uses these)
IMG_SIZE = (256, 256) # Resize target, adjust if your training used different
MEAN = [0.485, 0.456, 0.406]
STD = [0.229, 0.224, 0.225]

print("--- PyTorch Inference Script (MobileNetV3-Small) ---")

# --- Step 1: Set Device (Attempt ROCm GPU) ---
if torch.cuda.is_available():
    device = torch.device("cuda:0") # Use the first ROCm device
    try:
        gpu_name = torch.cuda.get_device_name(0)
        print(f"✅ ROCm GPU ({gpu_name}) found. Using GPU.")
    except Exception as e:
        print(f"✅ ROCm GPU found, but couldn't get name. Using GPU. Error: {e}")
        device = torch.device("cuda:0") # Still try to use it
else:
...
try:
    ...
    num_ftrs = model.classifier[3].in_features # Use index 3 for MobileNetV3-Small

    # Replace the LAST layer to match your number of classes
    model.classifier[3] = torch.nn.Linear(num_ftrs, NUM_CLASSES) # Use index 3
...
try:
    ...
    # Set strict=True first to catch mismatches. If it fails, try strict=False
    ...
    try:
        # Retry with strict=False if strict=True failed
        ...
        print("   Check if the keys in the state_dict truly match the MobileNetV3-Small architecture.")
        exit()
...
try:
    ...
    with torch.no_grad():
        _ = model(img_batch) # Run once without timing or storing output
    print("Warm-up complete.")

    # --- TIMED RUNS ---
    num_runs = 5 # Number of times to run for averaging
    latencies = []
    outputs = None # Initialize outputs to store the result from the last run
    ...
    for i in range(num_runs):
        ...
        # print(f"  Run {i+1}/{num_runs}: {latency:.2f} ms") # Optional: print each run's time
    ...
    result = {
        ...
        "latency_ms": round(avg_latency, 2) # Report the average latency
    }
